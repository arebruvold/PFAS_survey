...
hei <- raw_data %>% select(sample_id3) %>% mutate(sample_id_new = map_chr(sample_id3, mareano_renamer)) %>% filter(sample_id_new != sample_id3)

# importing curated LOQs
...
loqs <- left_join(
  ...
  mutate(
    sample_id = str_replace_all(sample_id, "BC", "BX"),
    # d/dd to ddd
    sample_id = if_else(str_detect(sample_id, "MC\\d{3}$|BX\\d{3}$"), sample_id,
      sample_id %>% str_replace("\\d{3}$", ~
                                paste()
                                )
    )
  ) %>%
  ...
  pivot_longer(
    cols = pfos:sum_9_pfas, names_to = "pfas", values_to = "pvas_value"
  ) %>%
  mutate(
    sample_id3 = str_replace_all(sample_id3, "\\s", ""),
    sample_id3 = str_replace_all(sample_id3, "BC", "BX")
  ) %>%
  distinct(sample_id3) %>%
  rename(sample_id = sample_id3)

library(arsenal)

cmp <- comparedf(loqs, raw_data_long, by = "sample_id")

cmp
summary(cmp)

cmp %>% diffs(what = "observations") %>% View()
...
* Model:
  - PFAS ~ area + TOC + clay/silt.
  - Test ZIP model, see picture and notes.
* Data:
  - Use Stepans LOQs. Go back and check raw data for values below LOD. Use LOQ/2.
  - Verdi for PFOA i prøve R2057 BX07 fra 2019 er angitt som 0,01 men er jo under LOQ <0,05
...
**sample_id3 == R3032** had a reported value that was below LOQ, but 
...
ggmap(tar_read(stadia_map))+
  geom_point(aes(x = longitude, y = latitude, color = area), data = raw_data, size = 2)
...
pfas_loqs <- tar_read(pfas_loqs)

# pivot 
raw_data %>% pivot_longer(matches("pf"), names_to = "pfas", values_to = "pfas_conc") %>% 
  left_join(pfas_loqs, by = c("pfas" = "pfas")) %>% 
  mutate(max_loq = if_else(max_loq %>% is.na(), 0, max_loq),
                           pfas_conc = ifelse(pfas_conc <= max_loq, max_loq/2, pfas_conc)) %>% 
  ggplot(aes(pfas_conc)) + geom_histogram(bins = 30)+facet_wrap(~pfas, scales = "free")
...
raw_data %>% 
  mutate(yhat = fitted(fit_lm),
         res = residuals(fit_lm)) %>%
  ggplot(aes(yhat, res)) + geom_point() + geom_smooth() + labs(title = "Residuals vs fitted")


raw_data %>% 
  mutate(yhat = fitted(fit_lm),
         res = residuals(fit_lm)) %>%
  ggplot(aes(clay_silt_fraction, res)) + geom_point() + geom_smooth() + labs(title = "Residuals vs covariate")

raw_data %>% 
  mutate(yhat = fitted(fit_lm),
         res = residuals(fit_lm)) %>%
  ggplot(aes(toc, res)) + geom_point() + geom_smooth() + labs(title = "Residuals vs covariate")

raw_data %>% 
  mutate(yhat = fitted(fit_lm),
         res = residuals(fit_lm)) %>%
  ggplot(aes(area, res)) + geom_point() + geom_smooth() +
...
raw_data %>% 
  mutate(yhat = fitted(fit_lm),
         res = residuals(fit_lm)) %>%
  ggplot(aes(year, res)) + geom_point() + geom_smooth() +
  stat_summary(fun = mean, geom = "point", color = "red", size = 3) +
  labs(title = "Residuals vs covariate")


ggplot(raw_data, aes(sample = sum_9_pfas)) +
  stat_qq(size = 0.2) +
...
library(performance)
# 
compare_performance(fit_lm, fit_glm, fit_glm_gamma)
...
https://www.youtube.com/watch?v=ISN9SE__QOU

Test dispersion, zero inflation.
* Residuals for temporal and spatial dependency
* Residuals vs fitted values
* Plot residuals vs each covariate in model
* Plot residuals vs each covariate not in model
* Plot the fitted values vs the observed values.
* See: https://sscc.wisc.edu/sscc/pubs/RegDiag-R/normality.html

Dharma residuals are the fraction of modelled values more extreme than this value.

We use ZIPs not ZAPs, as ZAPs essentialy causes us to split the dataset, and we have less possibility for an advanced model.

# ZIP Gamma model

```{r}
zipgamma <- glmmTMB((sum_9_pfas+0.01) ~ area + toc + clay_silt_fraction, data = raw_data, family = Gamma(link = "log"), ziformula = ~clay_silt_fraction)

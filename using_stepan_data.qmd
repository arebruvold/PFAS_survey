...
No substantial difference between the areas, SV is higher than NH, driven primarily by three high values.
...
pfas_data_eq %>% group_by(pfas, area) %>%
  ...
  gt(
    rowname_col = "pfas",
    groupname_col = "area"
  ) %>% tab_header(title = "Summary of PFAS values by area") %>% fmt_number(columns = 2:4,decimals = 2)
...
pfas_data_eq_wide %>% group_by(area) %>% 
  ...
  tab_header(title = "Summary of covariates by area") %>% 
  cols_label(
    area = md("**Area**"),

...
ggpairs(pfas_data_eq0_wide %>% 
          select(
Area, Mud, TOC, Σ9PFAS, PFOS, PFOA, PFNA, PFUnDA, PFDA, PFHpDA
          ),
        progress = FALSE)
```
...
```{r}
pfas_data_eq0_wide %>% select(-sample_id3, -TOC) %>% gt(groupname_col = "Area",rowname_col = "year") %>%
  fmt_number(columns = 3:15, n_sigfig = 2) %>%
  fmt_number(columns = longitude:latitude, n_sigfig = 5) %>% 
  cols_label(

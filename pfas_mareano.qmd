...
# VESTLAND BRANCH FOR PRESENTATION ONLY! (publication data different branch)
...
vestland_dataset <- read_excel("data/Marine Grunnkart Vestlandet 2023.xlsx")%>% filter(row_number() %in% seq(2,6)) %>% clean_names() %>% 
    select(sample_id3 = stasjon, latitude = n, longitude = e, TOC = toc, Mud = slam, matches("PF"), -sum_7_pfas) %>% 
  mutate(across(matches("PF"), as.numeric),
         Area = "V") %>%
  pivot_longer(matches("PF"), names_to = "pfas", values_to = "pfas_value") %>% 
  mutate(pfas = pfas_renamer(pfas))

vestland_loqs_raw <- read_excel("data/Marine Grunnkart Vestlandet 2023.xlsx") %>%
  clean_names() %>%
  filter(row_number() == 8) %>%
  select(-sum_7_pfas) %>% 
  mutate(across(everything(), as.numeric)) %>%
  select(matches("PF")) %>%
  pivot_longer(cols = everything(), names_to = "pfas", values_to = "loq") %>% 
  mutate(pfas = pfas_renamer(pfas))

vestland_raw <- left_join(vestland_dataset, vestland_loqs_raw, by = "pfas") %>% 
   mutate(across(-c("sample_id3", "Area", "pfas"), as.numeric)) %>% 
  filter(!(pfas %in% c("sum_7_pfas"))) %>% 
  mutate(pfas_value = if_else(is.na(pfas_value), 0, pfas_value)) %>% 
  mutate(year = 2024)
...
Arealat_wmodels <-
  pfas_data_eq0 %>%
  filter(!(pfas %in% c("PFHxA", "PFDoDA", "PFTrA"))) %>%
  ## inspect effect of excluding ares with high PFAS concentrations and high Mud content
  # filter(!(Area %in% c("I", "V"))) %>%
  mutate(
    # pfas = fct_relevel(pfas, c("Σ9PFAS", "PFOS", "PFOA", "PFNA", "PFUnDA", "PFDA","PFHpA" )),
    pfas_label = pfas
  ) %>%
  arrange(pfas) %>%
  group_by(pfas) %>%
...
tbl_merge((Arealat_wmodels %>% 
    filter(pfas %in% c("Σ9PFAS", "PFOS", "PFOA", "PFNA", "PFUnDA", "PFDA","PFHpA")))$reg_table, tab_spanner = c("Σ9PFAS", "PFOS", "PFOA", "PFNA", "PFUnDA", "PFDA","PFHpA")) %>% as_gt()

  
```

Applying the model with covariates 'C~{PFAS}~ = Mud + latitude', yields the significance of covariates shown in table above. Latitude is not a significant covariate for Σ9PFAS. Some trends in the individual PFASes point toward lower concentrations at higher latitudes for PFOS, PFUnDA, and PFDA, whereas PFOA may increase going northward. However, as shown in @fig-residuals-v-latitude, Areas I and V exert leverage on the data and local hotspots serve as hotspots driving much of the observed spatial variance. Thus, differences (and lack of thereof) may be driven largely by locations I and V. We also see that the variation is bigger for Areas I and V, and correspondingly, for low and high latitudes. This makes inference on the background levels difficult.
...
ggpairs(pfas_data_eq0_wide %>% 
          select(
            Area, Mud, TOC, Σ9PFAS, PFOS, PFOA, PFNA, PFUnDA, PFDA, PFHpA
          ),
        progress = FALSE)
```
...
# tbl_merge(pfas_w_models$reg_table, tab_spanner = c("Σ9PFAS", "PFOS", "PFOA", "PFNA", "PFUnDA")) %>% as_gt()
```
...
```{r loq-vs-stepan-check}

loq_check <- read_excel("data/temp_Antall over 0.xlsx") %>% clean_names() %>%
  rename(sample_id = sample
         ) %>%
  filter(!is.na(sample_id), !str_detect(sample_id, "Antall")) %>%
  mutate(sample_id = mareano_renamer(sample_id)) %>%
  pivot_longer(
    cols = c(pfos, pfna, pf_un_a),
    names_to = "pfas",
    values_to = "value"
  ) %>%
  mutate(sample_id = if_else(sample_id == "R2924BX001", "R2924BX002", sample_id)) %>%
  left_join(
    loqs, by = c("pfas", "sample_id")
  )
loq_check %>% glimpse

loq_check %>% 
  # mutate(
  #   across(c("loq", "value"), ~ .x %>%  round(digits = 6))) %>% 
  filter(value < loq & value > 0)

pfas_raw %>% 
  
  filter(pfas_value < loq & pfas_value > 0)
```
...
ggplot(pfas_data_eq0_wide, aes(Mud, TOC)) +
  geom_point(aes(fill = Area), color = "black", shape = 21) +

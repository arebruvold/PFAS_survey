...
---
# Todo
-   Finalize chemical properties table.
      - "name, structure, LogP, RT, m/z (or Mwt), chemical formula, and confidence level + class"
-   Tables to docx if needed.
-   Change CI on TOC vs Mud to Gamma, maybe others also.
-   Undersøke prøver mellom prøve-LOQ og høyeste LOQ, Stepan mener vurdere bruke invididuell LOQ.
-   Se stepan mail og rette.
...
pfas_data_eq0 <- pfas_raw %>%
  ...
  mutate(pfas = factor(pfas, levels = c("PFHxA","PFHpA", "PFOA", "PFNA", "PFDcA", "PFUnA", "PFDoA", "PFTriA", "PFOS", "Σ9PFAS")))
...
pfas_ratios <- pfas_data_eq0_wide %>%
  mutate(
    ...
    "PFDcA_to_PFUnA" = if_else(PFDcA == 0 | PFUnA == 0, NA, PFDcA / PFUnA)
  ) %>%
  select("PFOA_to_PFNA", "PFDcA_to_PFUnA",PFOA, PFNA, PFDcA, PFUnA, sample_id3:TOC) %>%
  pivot_longer(
    cols = c("PFOA_to_PFNA", "PFDcA_to_PFUnA"),
    names_to = "pfas",
    values_to = "Ratio"
  ) %>% mutate(c1 = if_else(pfas == "PFOA_to_PFNA", PFOA, PFDcA),
               c2 = if_else(pfas == "PFOA_to_PFNA", PFNA, PFUnA)
  ) %>% 
  mutate(pfas = case_when(
      pfas == "PFOA_to_PFNA" ~ "PFOA / PFNA",
      pfas == "PFDcA_to_PFUnA" ~ "PFDcA / PFUnA"
    )
  )
...
outlier_cand <- pfas_data_eq0_wide %>%
  ...
  filter(PFOA_to_PFNA > 3 | PFDcA_to_PFUnA < 0.3)

outlier_cand %>% gt() %>% fmt_number(columns = longitude:PFDcA_to_PFUnA)
...
ggplot(pfas_prop, aes(fill = pfas, y = pfas_prop, x = Area)) +
  geom_bar(position = "stack", stat = "identity", width = 0.88) +
...
between_maxloqloq <- pfas_raw %>%
  ...
  mutate(pfas = factor(pfas, levels = c("PFHxA","PFHpA", "PFOA", "PFNA", "PFDcA", "PFUnA", "PFDoA", "PFTriA", "PFOS", "Σ9PFAS"))) %>% group_by(pfas) %>% 
mutate(
...
between_maxloqloq %>% distinct(pfas, .keep_all = TRUE) %>% 
  select(
    pfas, n_between_loqs, n_above_max_loq, frac_between, min_loq, max_loq) %>% ungroup() %>% 
gt() %>% 

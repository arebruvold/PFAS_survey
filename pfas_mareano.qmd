...
```{r}
pfas_data_eq0_wide %>% select(-sample_id3) %>% gt(groupname_col = "Area",rowname_col = "year") %>%
  fmt_number(columns = 3:16, decimals = 2) %>%
  fmt_number(columns = longitude:latitude, decimals = 2) %>% 
  cols_label(
